...
describe("UCSBDiningCommonsMenuItemIndex tests", () => {
    ...
    test("renders three menu items correctly for regular user", async () => {
        setupUserOnly();
        axiosMock.onGet("/api/ucsbdiningcommonsmenuitems/all").reply(200, ucsbDiningCommonsMenuItemFixtures.threeItem);
        ...
        const createMenuButton = screen.queryByText("Create UCSB Dining Commons Menu Item");
        expect(createMenuButton).not.toBeInTheDocument();

        const name2 = screen.getByText("muffin");
        expect(name2).toBeInTheDocument();

        const station = screen.getByText("chef choice");
        expect(station).toBeInTheDocument();

        const station2 = screen.getByText("bakery");
        expect(station2).toBeInTheDocument();

        const station3 = screen.getByText("international");
        expect(station3).toBeInTheDocument();

        const code = screen.getByText("ortega");
        expect(code).toBeInTheDocument();

        const code2 = screen.getByText("dlg");
        expect(code2).toBeInTheDocument();

        const code3 = screen.getByText("portola");
        expect(code3).toBeInTheDocument();
    ...
    test("what happens when you click delete, admin", async () => {
        ...
        axiosMock.onGet("/api/ucsbdiningcommonsmenuitems/all").reply(200, ucsbDiningCommonsMenuItemFixtures.threeItem);
        axiosMock.onDelete("/api/ucsbdiningcommonsmenuitems").reply(200, "UCSBDiningCommonsMenuItem with id 1 deleted");
        ...
        await waitFor(() => { expect(mockToast).toBeCalledWith("UCSBDiningCommonsMenuItem with id 1 deleted") });
